...
// service account of this pod. The server then accepts one peer SPIFFE ID, and
// no other.
package main
...
const (
	...
	// makes a wrong service account easy to see in the log.
...
func newSource(serverID spiffeid.ID) (*workloadapi.X509Source, error) {
	...
	// SPIFFE_ENDPOINT_SOCKET. The pod sets it to
	// unix:///run/spire/sockets/agent.sock. The code names no socket path.
	...
	// SPIRE maps the service account of this pod to a SPIFFE ID. If the ID is
	// wrong, the pod uses the wrong service account. The client then refuses
